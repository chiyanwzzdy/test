...
function 生成链接(func) {
    ...
    if (mode == "zsf") {
        if (navigator.userAgent.indexOf("QQ/") != -1) {
            url = "https://h5.nes.smoba.qq.com/pvpesport.web.user/#/launch-game-mp-qq?gamedata=SmobaLaunch_"
        } else {
...
allbutton[0].onclick = function () {
    ...
    window.location.href = "tencentmsdk1104791911://?gamedata=SmobaLaunch_"
    /*
...
allbutton[1].onclick = function () {
    ...
    if (value == "zsf") {
        if (window.openurl) {
            ...
            var fxurl = encodeURIComponent(window.location.origin + "/open.html?openurl=" + url)
...
allbutton[2].onclick = function () {
    mdui.confirm({
        headline: "赛事群：621943852",
        description: "对战方式：1V1线上\n赛事赛制：匹配轮\n赛事创建时间与开赛时间：4月20日下午19点-21点\n冠亚季:30、20、10元\n十分钟人未到房间，默认弃权\n⚠️⚠⚠️反馈/领奖群:621943852",
        confirmText: "参赛",
        cancelText: "取消",
        onConfirm: () => {
            window.location.href = "https://h5.nes.smoba.qq.com/pvpesport.web.user/#/match/match-detail-index/42607712?adtag=wxsharechat"
        },
        onCancel: () => console.log("canceled"),
    });
}
...
allbutton[4].onclick = function () {
    window.location.href = "https://alimov2.a.kwimgs.com/bs2/photo-video-mz/1_5239938299342695322_sl200hd15_956d10481495cffd.25cebc.v0.mp4?tag=1-1712592659-unknown-0-swx964hiku-9f910e43de9c3b89&clientCacheKey=3xw86cmwcyaettq_6e35f818&di=279087dd&bp=10000&tt=sl200hd15&ss=vp"
}
allbutton[5].onclick = function () {
    mdui.confirm({
        headline: "提示",
        description: "点击确认 将跳转到所有开房网站的集合",
        confirmText: "确认",
        cancelText: "取消",
        onConfirm: () => {
            window.location.href = window.location.origin + "/allweb.html"
        },
        onCancel: () => console.log("canceled"),
    });
}
...
allbutton[6].onclick = function () {
    mdui.confirm({
        headline: "赞助作者",
        description: "赞助纯属自愿，金额多少都是心意，感谢您的支持",
        confirmText: "确认",
        cancelText: "取消",
        onConfirm: () => {
            window.location.href = window.location.origin + "/zanz.html"
        },
        onCancel: () => console.log("canceled"),
    });
}
